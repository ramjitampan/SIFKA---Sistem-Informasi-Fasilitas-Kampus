...
import { useBuildings, useCreateBuilding, useUpdateBuilding, useDeleteBuilding, Building } from '../hooks/useBuildings';
import { Building2, Plus, MapPin, Loader2, Edit, Trash2 } from 'lucide-react';
...
const BuildingsPage: React.FC = () => {
    ...
    const handleDeleteClick = (id: number) => {
        if (window.confirm(t('buildings.delete_confirm'))) {
            deleteBuilding(id, {
                onSuccess: () => toast.success(t('common.delete') + ' ' + t('common.success') || 'Deleted successfully'),
            });
        }
    };
    ...
    const handleFormSubmit = (data: Omit<Building, 'id'>) => {
        if (editingBuilding) {
            updateBuilding({ ...data, id: editingBuilding.id }, {
                onSuccess: () => {
                    toast.success(t('buildings.edit_building') + ' ' + t('common.success') || 'Updated successfully');
            ...
            createBuilding(data, {
                onSuccess: () => {
                    toast.success(t('buildings.add_building') + ' ' + t('common.success') || 'Created successfully');
                    setIsModalOpen(false);
                },
            });
        }
